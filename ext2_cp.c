...
int main(int argc, char **argv){
    ...
    // BEGIN Error checking after names assignment
    ...
    if(dest_fullpath_inode_num >= 1) {
        printf("%s: The destination file already exist.\n", dest_path);
        return ENOENT;
    }

    // END Error checking after names assignment
    ...
    if (bytes_copied < src_file_stat.st_size) {
        ...
        struct ext2_inode *indrect_inode = fetch_inode_from_num(dest_path_inode->i_block[12]);
        bytes_copied += copy_stream_to_inode(src_file_stream, indrect_inode);
        if (bytes_copied != src_file_stat.st_size) {
            printf("Bytes read does not match with file size");
            exit(-1);
        }
        dest_path_inode->i_size += indrect_inode->i_size;
        dest_path_inode->i_blocks += indrect_inode->i_blocks;
    }
